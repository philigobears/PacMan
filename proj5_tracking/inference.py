    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for b in self.beliefs:
            if self.beliefs[b] > 0 : 
                posDist = self.getPositionDistribution(self.setGhostPosition(gameState, b))
                for pos, prob in posDist.items(): 
                    allPossible[pos] += prob * self.beliefs[b]
        allPossible.normalize()
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles): 
            if pointer == len(self.legalPositions): 
                pointer -= len(self.legalPositions)
            self.particles.append(self.legalPositions[pointer])
            pointer += 1 
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None: 
            ...
            for p in self.particles: 
                allPossible[p] += emissionModel[util.manhattanDistance(p, pacmanPosition)]
            if allPossible.totalCount() == 0: 
                self.initializeUniformly(gameState) 
            else: 
                self.particles = [util.sample(allPossible) for i in range(0, len(self.particles))]
    ...
    def elapseTime(self, gameState):
        ...
        for i in range(len(self.particles)): 
            dist = self.getPositionDistribution(self.setGhostPosition(gameState, self.particles[i]))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        itor = itertools.product(self.legalPositions, repeat = self.numGhosts)
        self.particles = []
        index = 0 
        for i in range(self.numParticles): 
            if index == len(self.legalPositions) ** self.numGhosts: 
                index = 0 
                itor = itertools.product(self.legalPositions, repeat = self.numGhosts) 
            positions = itor.next()
            self.particles.append(positions) 
            index += 1
